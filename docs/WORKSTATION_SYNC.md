...
Double-click `sync-workstation.cmd`. Its editable defaults provide the private Drive `Projects` folder, expected Google account, and `main` branch. To use different values, edit the defaults in the batch file or define the corresponding `BRILLIANT_SYNC_*` environment variables before running it.

The first run saves the effective values outside the repository in:
