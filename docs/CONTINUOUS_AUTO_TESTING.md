...
- transient Apps Script timeout, redirect failure, or empty result body — coordinator protocol v4 assigns one request ID, stores the successful acknowledgement under the script lock, and safely replays that same request identity; duplicate claims, failure updates, and checkpoint writes are not created.
