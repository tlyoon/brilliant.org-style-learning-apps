...
class WorkstationSyncTests(unittest.TestCase):
    ...
    def test_validation_cache_tracks_repository_and_local_gemini_configuration(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            for relative in ("pyproject.toml", "requirements-generator.txt", "requirements-dev.txt"):
                (root / relative).write_text(relative + "\n", encoding="utf-8")
            source = root / PROJECT_CONFIG_RELATIVE_PATH
            source.parent.mkdir(parents=True)
            original = '[project]\nproject_name = "BrilliantContentGenerator"\n'
            ...
            settings.generated_config_file.write_text(
                '[local_gemini]\nlogin_name = "first@example.com"\n', encoding="utf-8"
            )

            with patch("scripts.sync_workstation._command", return_value="a" * 40):
                record_current_validation(settings)
                self.assertTrue(has_current_validation(settings))
                source.write_text(original + "# repository change\n", encoding="utf-8")
                self.assertFalse(has_current_validation(settings))
                record_current_validation(settings)
                self.assertTrue(has_current_validation(settings))
                settings.generated_config_file.write_text(
                    '[local_gemini]\nlogin_name = "second@example.com"\n', encoding="utf-8"
                )
                self.assertFalse(has_current_validation(settings))
                record_current_validation(settings)
                self.assertTrue(has_current_validation(settings))
                settings.generated_config_file.unlink()
                self.assertFalse(has_current_validation(settings))
