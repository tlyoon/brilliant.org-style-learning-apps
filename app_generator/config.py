...
def _flatten(document: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(local_gemini, Mapping):
    ...
    # Resolve legacy aliases and their shared OAuth fallback before applying
    # workstation overrides, so a local Gemini login never changes Drive identity.
    flat = _normalize_aliases(flat)
...
def load_config(
    ...
    *,
    ...
    with config_path.open("rb") as handle:
        file_values = _flatten(tomllib.load(handle))
    try:
