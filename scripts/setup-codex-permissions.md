...
6. Merge these narrow rules into `rules\default.rules`. Create the directory or
   file if necessary, preserve existing rules, and do not add duplicates:
...
7. Keep the safety boundary narrow:
   ...
   - Do not add broad PowerShell, Python, shell, deletion, package-installation,
     push, deployment, or unrestricted filesystem rules.
   - Do not read, copy, print, or modify credentials or authentication tokens.

8. Validate the resulting TOML and the three execution rules. Show the exact
